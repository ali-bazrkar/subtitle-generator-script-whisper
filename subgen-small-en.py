...
def main():
    ...
    # Dynamically locate models/small.en relative to script location
    script_dir = Path(__file__).parent.resolve()
    local_model_path = script_dir / "models" / "small.en"

    if local_model_path.exists():
        model_path = str(local_model_path)
        print("⏳ Loading local Whisper Small English model into VRAM (int8)...")
    else:
        model_path = "small.en"
        print(f"⚠️ Local model path '{local_model_path}' not found.")
        print("⏳ Falling back to default 'small.en' from Hugging Face hub...")
    ...
    for video_path in videos:
        ...
        try:
            ...
            print("🌡️ Cooling down for 8 seconds...")
            time.sleep(8)
